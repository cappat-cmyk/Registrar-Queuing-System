...
// Create SQL query with optional filters
$sql = "SELECT handled_By, client_type, studentID, fullname, email, is_priority, ticketNumber, transactionType, requestedDocument, department, status, remarksTextArea, additional_Info, DATE(arrivalTime), DATE(servedAt), DATE(finishedAt), avgwaitingtime, claimDate, is_claimed FROM transactionhistory WHERE 1=1";
...
header('Content-Disposition: attachment; filename="Filtered Transaction Details of Registrar Queuing System.csv"');
...
{ ob_end_clean();
    ...
    // Set response headers for file download
    header('Content-Type: text/csv');
    header('Content-Disposition: attachment; filename="Transaction Details of Registrar Queuing System.csv"');
